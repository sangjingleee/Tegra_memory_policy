...
def main() -> None:
    ...
    fig = plt.figure(figsize=(7.15, 2.85), dpi=240)
    ...
    for ax, (pattern, title) in zip(axes, PATTERNS):
        matrix = np.array([
            [data[(pattern, size, reuse)] for size in SIZES]
            for reuse in REUSES
        ])
    ...
    fig.subplots_adjust(left=0.065, right=0.965, bottom=0.20, top=0.90)

    out_png = OUT_DIR / "fig3_heatmap_dense.png"
    out_pdf = OUT_DIR / "fig3_heatmap_dense.pdf"
