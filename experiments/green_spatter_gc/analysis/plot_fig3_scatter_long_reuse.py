...
DATA_CANDIDATES = [
    ROOT / "results_scatter_long_reuse.csv",
    ROOT / "remote_archive_20260720" / "green_ctx_0522" / "results_scatter_long_reuse.csv",
]
DATA = next((path for path in DATA_CANDIDATES if path.exists()), DATA_CANDIDATES[0])
OUT_DIR = ROOT / "paper_figures"

SIZES = [1, 4, 16, 64]
REUSES = [512, 256, 128, 64]
...
def load_rows():
    ...
    out = {}
    for row in rows:
        key = (int(float(row["size_mb"])), int(float(row["reuse"])))
...
def main() -> None:
    ...
    matrix = np.array([[data[(size, reuse)] for size in SIZES] for reuse in REUSES])

    fig = plt.figure(figsize=(3.2, 2.55), dpi=240)
    gs = fig.add_gridspec(1, 2, width_ratios=[1, 0.07], wspace=0.14)
    ...
    ax.set_title("Scatter long-reuse", pad=4, fontweight="bold")
    ax.set_xticks(range(len(SIZES)))
    ax.set_xticklabels([str(size) for size in SIZES])
    ax.set_yticks(range(len(REUSES)))
    ax.set_yticklabels([str(reuse) for reuse in REUSES])
    ...
    fig.subplots_adjust(left=0.17, right=0.93, bottom=0.19, top=0.90)

    out_png = OUT_DIR / "fig3_scatter_long_reuse.png"
    out_pdf = OUT_DIR / "fig3_scatter_long_reuse.pdf"
