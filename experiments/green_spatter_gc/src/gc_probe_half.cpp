...
int main(int argc, char** argv){
  ...
  int half = argc>3 ? atoi(argv[3]) : 0;     // 0: first split, 1: remaining split
  CK(cuInit(0));
  ...
  CUdevResource* selected = half ? &rem : &g;
  CUdevResourceDesc d; CK(cuDevResourceGenerateDesc(&d,selected,1));
  ...
  std::printf("PID %d | half=%d | greenctx smCount=%u | physical SMs used: ",
              (int)getpid(), half, act.sm.smCount);
