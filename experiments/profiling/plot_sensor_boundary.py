...
MODES = ["pageable_copy", "pinned_copy", "mapped_zc", "managed", "device_preloaded"]
COLORS = {
    "pageable_copy": "#d94841",
    "pinned_copy": "#f0a33a",
    "mapped_zc": "#3c9d5a",
    "managed": "#3b82c4",
    "device_preloaded": "#555555",
}
...
def draw_grouped(rows, out, title, size_mb):
    ...
    w, h = max(980, 245 * len(contentions)), 560
    left, top, bottom = 85, 70, 80
    plot_w, plot_h = w - left - 35, h - top - bottom
    vmax = max(r["total_ms"] for r in subset) * 1.15
    ...
    parts = [
        ...
        f'<text x="22" y="{top + plot_h/2}" transform="rotate(-90 22 {top + plot_h/2})" text-anchor="middle" font-size="15" font-family="Arial">Total latency (ms)</text>',
    ]
    for i in range(6):
        yv = vmax * i / 5
    ...
    for gi, cont in enumerate(contentions):
        x0 = left + gi * group_w
        label = cont.replace("model_", "").replace("mobilenetv2", "mbv2").replace("mixed_managed", "mixed").replace("default", "def")
        parts.append(f'<text x="{x0 + group_w/2}" y="{h - 40}" text-anchor="middle" font-size="12" font-family="Arial">{label}</text>')
    ...
    for mode in MODES:
        ...
        parts.append(f'<text x="{lx+17}" y="{ly}" font-size="12" font-family="Arial">{mode}</text>')
        lx += 170
    parts.append("</svg>")
...
def main():
    ...
    p.add_argument("--csv", default="sensor_boundary_contention_orin.csv")
    p.add_argument("--prefix", default="sensor_boundary_orin")
    args = p.parse_args()
    rows = read_rows(Path(args.csv))
    for size in [4, 16]:
        draw_grouped(
            rows,
            Path(f"{args.prefix}_{size}mb_total.svg"),
            f"Orin-A Sensor Boundary Total Latency ({size}MB)",
            size,
        )
